...
exports.getRepos = function(){
  ...
  $.get('https://api.github.com/users/' + username + '/repos?access_token=' + apiKey + '&per_page=1000&sort=created').then(function(repos){
    ...
    $('.repo-results').append("<tr><th>Repository</th><th>Description</th><th>Created At</th></tr>");
    for(var i = 0; i < repos.length ; i++) {
      $('.repo-results').append("<tr><td><a href='https://github.com/" + username + "/" + repos[i].name + "'>" + repos[i].name + "</a></td><td>" + repos[i].description + "</td><td>" + repos[i].created_at + "</td></tr>");
    };
  }).fail(function(error){
    console.log(error.responseJSON.message);
  });
